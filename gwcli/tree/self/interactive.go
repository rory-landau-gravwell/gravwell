...
// This file implements interactive versions of self actions: change-password, search-group, and update.
	...
	cmd := treeutils.GenerateAction("search-group", "get or set default search groups",
		...
		func(c *cobra.Command, args []string) error {
			...
			if c.Flags().Changed("set") {
				setVal, err := c.Flags().GetString("set")
				if err != nil {
					clilog.GetFlag(err)
					return err
				}
				return handleSearchGroupSet(c, uid, setVal)
			}

			// if no set flag, try interactive mode
			ni, err := c.Flags().GetBool(ft.NoInteractive.Name())
			if err != nil {
				clilog.GetFlag(err)
				ni = true
			...
			fmt.Fprintf(c.OutOrStdout(), "default search groups: %v\n", gids)
			return nil
		},
	)
	cmd.Flags().String("set", "", "comma-separated list of group IDs to set as default")
...
func (c *searchGroupModel) View() string { return c.m.View() }
func (c *searchGroupModel) Done() bool   { return c.m.Done() }
func (c *searchGroupModel) Reset() error { c.m = multiselectlist.Model[int32]{}; return nil }

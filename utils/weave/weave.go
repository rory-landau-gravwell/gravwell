...
const (
	...
	ErrEmptyPackageName string = "package "
)
...
func main() {
	pkgIdentifier := regexp.MustCompile(`^[\pL_]\w*$`)

	// process and validate inputs
	var pkg string
	flag.StringVar(&pkg, "-package", "", "the package name the generated code should use")
	flag.Parse()
	if strings.TrimSpace(pkg) == "" {
		fmt.Println("--package must be set")
		os.Exit(1)
	} else if !pkgIdentifier.MatchString(pkg) {
		...
		fmt.Println("--package must be a valid go identifier." +
	...
	// fetch stringifications for each type in the file
	// TODO
